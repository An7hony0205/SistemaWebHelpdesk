...
<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-bold text-content">Mis Preferencias de Notificación</h2>
    </div>
    ...
    <div v-else class="bg-surface rounded-lg shadow p-6">
      <div class="space-y-6">
        <div v-for="eventName in availableEvents" :key="eventName" class="border-b pb-4 last:border-0 last:pb-0">
          <h3 class="text-lg font-medium text-content">{{ eventName }}</h3>
          <p class="text-sm text-muted mb-4">Selecciona los canales por los que deseas recibir notificaciones cuando ocurra este evento.</p>
          
          <div class="flex items-center space-x-6">
            <label v-for="channel in availableChannels" :key="channel" class="inline-flex items-center">
              <input 
                type="checkbox" 
                :value="channel" 
