...
<template>
  <div class="space-y-6 font-body">
    <div>
      <h2 class="text-2xl font-bold font-brand text-content">Integraciones & Desarrolladores</h2>
      <p class="text-sm text-muted mt-1">Conecta tu HelpDesk con sistemas externos mediante API y Webhooks.</p>
    </div>
    ...
    <div v-else-if="activeTab === 'apikeys'" class="space-y-4">
      <div class="flex justify-end">
        <HdButton @click="generateApiKey" variant="primary">
          Generar API Key
        </HdButton>
      </div>

      <HdCard :noPadding="true">
        <HdTable :columns="apiKeyColumns" :items="apiKeys">
          ...
          <template #cell(actions)="{ item }">
            <HdButton variant="ghost" size="sm" class="text-danger" @click="deleteApiKey(item.id)">
              Revocar
            </HdButton>
          </template>
        </HdTable>
    ...
    <div v-else-if="activeTab === 'webhooks'" class="space-y-4">
      <div class="flex justify-end">
        <HdButton @click="createWebhook" variant="primary">
          Añadir Webhook
        </HdButton>
      </div>

      <HdCard :noPadding="true">
        <HdTable :columns="webhookColumns" :items="webhooks">
          <template #cell(url)="{ value }">
            <span class="font-medium text-content">{{ value }}</span>
          </template>
          <template #cell(events)="{ value }">
            <div class="flex flex-wrap gap-1">
              <HdBadge v-for="ev in value" :key="ev.id" variant="neutral">{{ ev.event_name }}</HdBadge>
            </div>
          </template>
