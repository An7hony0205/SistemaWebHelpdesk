...
<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-bold font-brand text-content">Dashboard Ejecutivo</h2>
      <div class="flex space-x-2">
        <HdButton @click="fetchMetrics" variant="secondary" size="sm">
          <svg v-if="loadingMetrics" class="animate-spin -ml-1 mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          <svg v-else class="-ml-1 mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
          {{ loadingMetrics ? 'Actualizando...' : 'Actualizar' }}
        </HdButton>
      </div>
    </div>

    <div v-if="loadingLayout" class="flex justify-center py-12">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <template v-for="widget in layout" :key="widget.id">
        ...
        <div v-if="widget.id === 'tickets_by_status'" class="col-span-1 md:col-span-2 lg:col-span-2">
          <KpiDoughnutChart 
