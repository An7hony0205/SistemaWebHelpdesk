  ...
  <HdCard :noPadding="true">
    <template #header>
      <div class="flex justify-between items-center">
        <h3 class="text-lg font-medium text-content">Todos los Tickets</h3>
        <HdButton @click="showModal = true" variant="primary" size="sm">
          Nuevo Ticket
        </HdButton>
      </div>
    </template>
    
    <div v-if="loading" class="p-6 text-center text-muted font-body">
      Cargando tickets...
    </div>
    
    <div v-else-if="filteredTickets.length === 0" class="p-6 text-center text-muted font-body">
      No hay tickets disponibles.
    </div>

    <!-- Kanban View -->
    <div v-else-if="prefsStore.tickets.default_view === 'kanban'" class="p-6 font-body">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Tableros de ejemplo simplificados -->
        <div class="bg-surface-elevated p-4 rounded-lg border border-subtle">
          <h4 class="text-muted font-medium mb-4">Abiertos</h4>
          <div v-for="ticket in filteredTickets.filter(t => t.status === 'Abierto')" :key="ticket.id" @click="goToTicket(ticket.id)" class="bg-surface p-3 rounded-md mb-2 shadow-base cursor-pointer border border-subtle hover:bg-black/5 transition-colors">
            <p class="text-sm text-content font-medium">#{{ ticket.id }} {{ ticket.title }}</p>
            <p class="text-xs text-muted mt-2">{{ new Date(ticket.created_at).toLocaleDateString() }}</p>
          </div>
        </div>
        <div class="bg-surface-elevated p-4 rounded-lg border border-subtle">
          <h4 class="text-muted font-medium mb-4">En Progreso / Otros</h4>
          <div v-for="ticket in filteredTickets.filter(t => t.status !== 'Abierto' && t.status !== 'Cerrado' && t.status !== 'Resuelto')" :key="ticket.id" @click="goToTicket(ticket.id)" class="bg-surface p-3 rounded-md mb-2 shadow-base cursor-pointer border border-subtle hover:bg-black/5 transition-colors">
            <p class="text-sm text-content font-medium">#{{ ticket.id }} {{ ticket.title }}</p>
            <p class="text-xs text-muted mt-2">{{ new Date(ticket.created_at).toLocaleDateString() }}</p>
          </div>
        </div>
        <div class="bg-surface-elevated p-4 rounded-lg border border-subtle">
          <h4 class="text-muted font-medium mb-4">Cerrados</h4>
          <div v-for="ticket in filteredTickets.filter(t => t.status === 'Cerrado' || t.status === 'Resuelto')" :key="ticket.id" @click="goToTicket(ticket.id)" class="bg-surface p-3 rounded-md mb-2 shadow-base cursor-pointer border border-subtle opacity-70 hover:bg-black/5 transition-colors">
            <p class="text-sm text-content font-medium line-through">#{{ ticket.id }} {{ ticket.title }}</p>
            <p class="text-xs text-muted mt-2">{{ new Date(ticket.created_at).toLocaleDateString() }}</p>
          </div>
        </div>
      </div>
    ...
    >
      <template #cell(id)="{ value }">
        <span class="font-medium cursor-pointer text-primary hover:underline" @click="goToTicket(value)">#{{ value }}</span>
      </template>
        ...
        <div class="relative z-10 inline-block align-bottom bg-surface rounded-lg text-left overflow-hidden shadow-modal border border-subtle transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <form @submit.prevent="createTicket">
            <div class="px-6 pt-5 pb-4">
              <h3 class="text-lg leading-6 font-medium font-brand text-content mb-4">Crear Nuevo Ticket</h3>
              <div class="space-y-4">
                <HdInput 
